 ...
 */
@SuppressWarnings({"unchecked", "UnnecessaryBoxing", "ManualMinMaxCalculation", "UnnecessaryUnboxing"})
public class TextComponent extends Widget {
	...
	private void drawLine(int index) {
		...
		if ((firstChar < length) && ((index - firstLine) < rect.getHeight())) {
			...
			Toolkit.printString(
					replaceTextLineForPrinting(_text.substring(pos + firstChar, pos + firstChar + length3)),
